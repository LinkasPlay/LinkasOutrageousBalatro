...
-- Fonction utilitaire pour éviter les répétitions et les coupures pendant les mains
...
-- MUSIQUE DES BOSS "IRL"
SMODS.Sound {
    key = "music_irl_boss",
    path = "irl_boss.ogg",
    pitch = 1, -- Force le pitch normal
    select_music_track = function()
        if G.GAME and G.GAME.blind and is_in_battle_state() then
            ...
            if is_boss then return 100 end -- Priorité numérique pour Penumbra
        end
    end
}
    ...
    press_play = function(self)
        if not G.GAME.blind.disabled then
            ...
            for i = 1, math.min(num_to_debuff, #valid_cards) do
                -- On ajoute 'i' à la seed pour garantir l'aléatoire à chaque boucle
                ...
                if random_card then
                    ...
                    -- On cherche l'index manuellement pour la retirer des cibles
...
SMODS.Blind {
    ...
    drawn_to_hand = function(self, card)
        ...
        -- Si l'alias de défausse a flagué qu'on est en train de repiocher suite à un discard
...
-- =================================================================
-- HOOKS GLOBAUX (Temps réel et Affichage Géant)
-- =================================================================
...
function Game.update(self, dt)
    ...
    if G.GAME and G.STATE == G.STATES.SELECTING_HAND and G.GAME.blind then
        -- UTILISEZ LA CLEF INTERNE POUR LA SECURITE
            ...
            if G.GAME.lob_renoir_timer >= 3 then
                ...
                if G.hand and G.hand.cards then
                    for i = 1, #G.hand.cards do
                        ...
                        -- On vérifie le tag lob_renoir_debuff
                ...
                if best_card then
                    best_card.lob_renoir_debuff = true -- On marque la carte !
        ...
        if boss_key == 'bl_lob_boss_speedrun' and not G.GAME.blind.disabled then
            ...
            if G.GAME.lob_speedrun_timer >= 1 then 
                ...
                if G.GAME.lob_speedrun_base_chips then
                    -- Calcul linéaire : +1% du score initial par seconde
...
function Game.draw(self)
    ...
    if G.GAME and G.GAME.blind and G.GAME.blind.name == 'boss_anne_christine' and not G.GAME.blind.disabled then
        if G.STATE == G.STATES.SELECTING_HAND or G.STATE == G.STATES.PLAY_TAROT then
            -- On vérifie que l'atlas existe
            if G.ASSET_ATLAS["lob_anne_geante"] then
                local tex = G.ASSET_ATLAS["lob_anne_geante"]
                if tex.image then
                    love.graphics.setColor(1, 1, 1, 0.8)
                    
                    -- CORRECTION : Utilisation de love.graphics pour obtenir les dimensions réelles
                    local screen_w = love.graphics.getWidth()
                    local screen_h = love.graphics.getHeight()
                    
                    -- Calcul de l'échelle (scale) pour que l'image remplisse l'écran
                    -- tex.px et tex.py sont les dimensions de l'image dans l'atlas
